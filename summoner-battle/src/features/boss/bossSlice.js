...
function calculateAttackDamage(player, situation) {
  ...
  if (Date.now() % 100 < skills.CRP[player.luck] * 100) {
    (finalDamage *= skills.CRD[player.power]);
  }
  console.log(`${finalDamage}의 데미지!`);
  return Math.floor(finalDamage);
}

function calculateAtkCriDamage(player, situation) {
  let finalDamage = player.damage * 10;
  if (situation.element === 'dark' || situation.element === 'light') {
    finalDamage += skills.DLD[player[situation.element]];
  } else {
  ...
  console.log(`${finalDamage}의 데미지(크리티컬) 취소!`);
  return Math.floor(finalDamage);
}

function calculateNormalDamage(player, situation) {
  let finalDamage = player.damage * 10;
  if (situation.element === 'dark' || situation.element === 'light') {
    finalDamage += skills.DLD[player[situation.element]];
  } else {
  ...
  console.log(`${finalDamage}의 데미지(일반) 취소!`);
  return finalDamage;
}
...
export const battleSlice = createSlice({
  ...
  reducers: {
    ...
    attackCancel: (state, action) => {
      const boss = state.boss;
      state.boss.hp += calculateNormalDamage(state.player, action.payload);
      if (state.boss.hp < boss.p4) {
        state.boss.phase = 5;
      } else if (state.boss.hp < boss.p3) {
        state.boss.phase = 4;
    ...
    atkCriCancel: (state, action) => {
      const boss = state.boss;
      state.boss.hp += calculateAtkCriDamage(state.player, action.payload);
      if (state.boss.hp < boss.p4) {
        state.boss.phase = 5;
      } else if (state.boss.hp < boss.p3) {
        state.boss.phase = 4;
...
export const selectBoss = (state) => state.boss.boss;
export const selectPlayer = (state) => state.boss.player;
export const selectSummon = (state) => state.boss.summon;
